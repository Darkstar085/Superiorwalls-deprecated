...
import com.sipun.superiorwalls.library.data.db.SuperiorwallsDatabase
...
import com.sipun.superiorwalls.library.data.network.WallpapersJSONService
...
import com.sipun.superiorwalls.library.extensions.utils.postDelayed
import com.sipun.superiorwalls.library.extensions.utils.tryToObserve
...
open class WallpapersDataViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private suspend fun getWallpapersFromDatabase(): List<Wallpaper> = withContext(IO) {
        try {
            SuperiorwallsDatabase.getAppDatabase(context)?.wallpapersDao()
    ...
    private fun internalAddToLocalFavorites(wallpapers: List<Wallpaper>): Boolean {
        SuperiorwallsDatabase.getAppDatabase(context)?.favoritesDao()
            ?.insertAll(wallpapers.map { Favorite(it.url) })
        return true
    }

    private fun internalNukeAllLocalFavorites(): Boolean {
        SuperiorwallsDatabase.getAppDatabase(context)?.favoritesDao()?.nuke()
        return true
    }

    open suspend fun internalGetFavorites(): List<Favorite> =
        SuperiorwallsDatabase.getAppDatabase(context)?.favoritesDao()?.getAllFavorites().orEmpty()

    open suspend fun internalAddToFavorites(wallpaper: Wallpaper): Boolean {
        SuperiorwallsDatabase.getAppDatabase(context)?.favoritesDao()?.insert(Favorite(wallpaper.url))
        return true
    }

    open suspend fun internalRemoveFromFavorites(wallpaper: Wallpaper): Boolean {
        SuperiorwallsDatabase.getAppDatabase(context)?.favoritesDao()?.delete(Favorite(wallpaper.url))
        return true
    }
    ...
    private suspend fun getNextWallpaperInCollection(
        ...
        val wallpapersDao = SuperiorwallsDatabase.getAppDatabase(context)?.wallpapersDao()
    ...
    suspend fun getNextFavoriteWallpaper(currentUrl: String): Wallpaper? = withContext(IO) {
        val db = SuperiorwallsDatabase.getAppDatabase(context)
